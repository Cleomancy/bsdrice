...
pkg install -y xorg-server xinit libxinerama libxft feh unclutter unzip font-awesome hack-font ncurses pkgconf
echo 'hald_enable="YES"' >> /etc/rc.conf
echo 'dbus_enable="YES"' >> /etc/rc.conf
printf "installed dependencies\n"

mkdir /usr/local/etc/rice/
mv $userhome/rice/rice.zip /usr/local/etc/rice/
